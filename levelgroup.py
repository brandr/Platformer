...
#Dungeon currently represents all the levels in the game. If for some reason it becomes undesirable
    ...
    #fit neatly into one grid, etc) then we might make it possible to travel between Dungeons.

class Dungeon(object):
	def __init__(self, level_data_set, room_data_set): #Dungeon builds the dungeon from a single map (currently ascii) along with some other data about the level.
		#new init
	...
	def movePlayer(self, player, next_level, global_coords, local_coords):
		room_coords = (local_coords[0] % ROOM_WIDTH,local_coords[1] %R OOM_HEIGHT)
