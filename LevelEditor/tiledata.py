...
class TileData(object):
	...
	def __init__(self, key, filepath = None):
		...
		self.setImage(key,filepath) #TODO: may need a filepath arg here.

	def setImage(self,key,filepath = None):
		filename_start = "./images/"
		if filepath != None:
			filename_start = filepath
		filename_end = SELECTABLE_ENTITY_MAP[key]
		entity_image = Image.load_image (filename_start+filename_end) #this directory doesn't seem to work if we are actually in the LeveEditor.
	...
	@staticmethod
	def deformatted_tile_set(formatted_data,filepath = None):
		tiles = []
		for y in xrange (len(formatted_data)):
			...
			for x in xrange(len(formatted_data[y])):
				...
				if next_tile != None:
					next_data = TileData.deformatted_tile(next_tile,filepath)
				tiles[y].append(next_data)
		return tiles

	@staticmethod
	def deformatted_tile(formatted_data,filepath = None):	#this will need to change as this class's constructor does.
		return TileData(formatted_data,filepath)